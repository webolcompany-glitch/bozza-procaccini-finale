...
def invia_email(destinatari, prezzo, template, nome=""):
    try:
        ...
        scarico = calcola_data_scarico()   # 👈 AGGIUNTO

        testo = template \
            .replace("{prezzo}", f"{prezzo:.3f}") \
            .replace("{nome}", nome) \
            .replace("{data}", data) \
            .replace("{scarico}", scarico)   # 👈 AGGIUNTO

        msg = MIMEMultipart("alternative")

        # 👇 MODIFICA OGGETTO EMAIL
        msg["Subject"] = f"OFFERTA CARBURANTE - SCARICO {scarico}"
